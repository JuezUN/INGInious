...
def project_detail_best_user_tasks(user_tasks, summary_result):
    ...
    } for s in user_tasks if (s.get("submission", {}).get("custom", {}).get("custom_summary_result", "ACCEPTED" if (s.get("submission", {}).get("result", {}) == "success") else "WRONG_ANSWER") == summary_result)]

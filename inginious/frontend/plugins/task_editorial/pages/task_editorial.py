...
def editorial_task_preview(course, task, template_helper):
    ...
    content = template_helper.get_custom_renderer(_TASK_EDITORIAL_TEMPLATE_PATH, layout=False).task_editorial_preview(course, task, is_task_open(task), get_all_available_languages(), task_tutorial_description, task_solution_code, task_solution_code_language, task_solution_code_notebook, task_environment)
    return str(content)

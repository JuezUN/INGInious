...
def editorial_task_preview(course, task, template_helper):
    ...
    content = template_helper.get_custom_renderer(_TASK_EDITORIAL_TEMPLATE_PATH, layout=False).task_editorial_preview(course, task, is_task_open(task), get_all_available_languages(), task_tutorial_description, task_solution_code, task_solution_code_language, task_environment)
    return str(content)
...
def check_editorial_submit(course, taskid, task_data, task_fs):
    ...
    if task_data['environment'] in {"multiple_languages" , "Data Science" , "HDL"}:
    ...
    elif task_data['environment'] is "Notebook":
        ...
        if solution_code_notebook not in all_files:
